...
def read_data_loop():
    ...
    while run_flag:
        try:
            ...
            if cali_status == "done":
                if (
                    (cliff_reference[0] < line_reference[0])
                    and (cliff_reference[1] < line_reference[1])
                    and (cliff_reference[2] < line_reference[2])
                ):
                    cliff_reference[0] = int(
                        (cliff_reference[0] + line_reference[0]) / 2
                    )
                    cliff_reference[1] = int(
                        (cliff_reference[1] + line_reference[1]) / 2
                    )
                    cliff_reference[2] = int(
                        (cliff_reference[2] + line_reference[2]) / 2
                    )
                cali_status = "none"
...
def start_cliff_calibrate():
    def cliff_calibrate_work():
        ...
        if (
            _left_val < thresholds[0][0]
            and _mid_val < thresholds[1][0]
            and _right_val < thresholds[2][0]
        ):
