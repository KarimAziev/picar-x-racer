export interface KeyboardKeyProps
  extends Pick<KeyboardEventInit, "code" | "keyCode"> {
  key: string;
  text?: string;
  location?: number;
  shiftKey?: boolean;
...
export class KeyboardMapper {
  ...
  static nonModifiersConfig: { [key: string]: KeyboardKeyProps } = {
    Power: {
      key: "Power",
      code: "Power",
    },
  ...
  public static translateKeyboardEventToKey(
    event: Pick<KeyboardEvent, "code" | "key">,
  ) {
    const { key, code } = event;
    if (KeyboardMapper.nonModifiersConfig[key]) {
      return key;
    }
    const props = KeyboardMapper.nonModifiersConfig[code];
